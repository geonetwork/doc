...
.. _common_advanced_editor_fields:

Common advanced editor fields
-----------------------------

This section presents a number of common advanced editor fields.

*data-gn-date-picker*

Fields of type date benefit from the datepicker fieldtype. You can configure what type of dates are allowed, eg. year, date, datetime.

*data-gn-anchor-switcher*

Facilitates easy conversion between Characterstring and Anchor.

*data-gn-keyword-picker*

Configure a thesaurus from which the concepts are used to populate an auto complete suggestion for this field. 
Selected value is stored as Characterstring or Anchor.

.. code-block:: xml

    <for name="gmd:protocol" use="data-gn-keyword-picker">
      <directiveAttributes 
        data-thesaurus-key="external.theme.protocols" 
        data-order-by-id="true"
        data-display-definition="true"
        data-thesaurus-concept-id-attribute="xlinkCOLONhref"/>
    </for>

*data-gn-logo-selector*

Users can upload a file into the file storage of a record. The link to the file will be stored as Anchor.

*data-gn-language-picker, data-gn-country-picker, data-gn-crs-selector*

Users can select a language, country or crs from a pull down.
...
but can also be used to store snippets of xml having predefined Quality reports, Access constraints, CRS definitions, etc. 
...
*data-gn-thesaurus-selector*

Facilitates to select a keyword from a thesaurus.